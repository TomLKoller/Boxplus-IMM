...
#ifndef NAIVE_RTS_IMM_H
#define NAIVE_RTS_IMM_H
...
namespace adekf
{
    ...
    public:
         ...
         */
        Naive_RTSIMM(const State &_mu, const Covariance &_sigma, std::initializer_list<DYNAMIC_NOISE_TYPE> dyn_noises, Models... models) : mu(_mu), sigma(_sigma),
                                                                                                                                       model_bank(models...)
        {
        ...
        State weightedStateSum(const Eigen::Matrix<double, -1, 1> &probabilities, const State &last, const GetState &stateGetter = getCurrentState)
        {
            ...
            for (size_t i = 0; i < numFilters(); i++) {
                if (probabilities(i) < 0.)
                    ...
                    State state = stateGetter(filter_bank[i].first);
  ...
  */
    ...
    Naive_RTSIMM(const DERIVED &, const COV_TYPE &, std::initializer_list<DYN_COV>,
             Models...) -> Naive_RTSIMM<Naive_RTS_Smoother, typename StateInfo<DERIVED>::type, DYN_COV, Models...>;
    ...
    auto createBPRTSIMM(const STATE_TYPE &mu, const COV_TYPE &sigma, Models... models) -> Naive_RTSIMM<Smoother, typename StateInfo<STATE_TYPE>::type, Models...>
    {
        return Naive_RTSIMM<Smoother, typename StateInfo<STATE_TYPE>::type, Models...>(mu, sigma, models...);
    }

}

#endif //NAIVE_RTS_IMM_H
